...
#Train and Eval dm_control walker walk task with RNN SAC.
...
_root_dir = 'dm_control_walker_walk_training_app/'

def convert_time_step(time_step):
  step_type = tf.convert_to_tensor(tf.Variable([0], shape=(None,)), dtype=tf.int32, name='step_type')
  reward = tf.convert_to_tensor(tf.Variable([0], shape=(None,), dtype=tf.float32), dtype=tf.float32, name='reward')
  discount = tf.convert_to_tensor(tf.Variable([1], shape=(None,), dtype=tf.float32), dtype=tf.float32, name='discount')
  observations = tf.convert_to_tensor(tf.Variable(time_step[3].numpy(), shape=(None, 17), dtype=tf.float32), dtype=tf.float32, name='observations')
...
def create_video(py_environment: PyEnvironment, tf_environment: TFEnvironment, policy, num_episodes=1, video_filename='imageio.mp4'):
  ...
  with imageio.get_writer(video_filename, fps=60) as video:
    for episode in range(num_episodes):
      ...
      #time_step = convert_time_step(tf_environment.reset())
...
#flags.DEFINE_string('root_dir', 'sac/dog_run',
#                    'Root directory for writing logs/summaries/checkpoints.')
flags.DEFINE_string('root_dir', _root_dir,
                    'Root directory for writing logs/summaries/checkpoints.')
...
def train_eval(
    ...
    observations_allowlist=['orientations', 'height', 'velocity'],
    #observations_allowlist=['joint_angles', 'joint_velocites', 'torso_pelvis_height', 'z_projection', 'torso_com_velocity', 'inertial_sensors', 'foot_forces', 'touch_sensors', 'actuator_state'],
    #observations_allowlist='joint_angles,joint_velocites,torso_pelvis_height,z_projection,torso_com_velocity,inertial_sensors,foot_forces,touch_sensors,actuator_state',

    #observations_allowlist=None,
    eval_env_name=None,
    ...
    for _ in range(num_iterations):
      ...
      for _ in range(episode_steps):
        ...
        if global_step_val % policy_checkpoint_interval == 0:
          ...
          #create_video(eval_py_env, eval_tf_env, tf_agent.policy, num_episodes=1, video_filename=os.path.join(videos_dir, "video_%d.mp4" % global_step_val))
          create_video(eval_py_env, eval_tf_env, eval_policy, num_episodes=2, video_filename=os.path.join(videos_dir, "video_%d.mp4" % global_step_val))
